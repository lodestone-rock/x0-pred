...
def warmup_compiled_model(
    ...
    """
    cap_feat_dim       = model_cfg.get("cap_feat_dim", 2560)
    spatial_patch_size = model_cfg.get("spatial_patch_size", 32)
    ...
    for gpu_id, model in enumerate(models):
        ...
        with torch.no_grad(), torch.autocast("cuda", torch.bfloat16):
            for H, W in sorted(image_shapes):
                ...
                dummy_txt = torch.zeros(1, max_seq_len, cap_feat_dim, device=device)
                dummy_msk = torch.ones(1, max_seq_len, dtype=torch.bool, device=device)
                model(dummy_img, dummy_t, dummy_txt, dummy_msk)
                num_patches = (H // spatial_patch_size) * (W // spatial_patch_size)
